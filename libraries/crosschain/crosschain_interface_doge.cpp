...
namespace graphene {
	namespace crosschain {
		...
		void crosschain_interface_doge::initialize_config(fc::variant_object &json_config)
		{
			_config = json_config;
			_rpc_method = "POST";
			_rpc_url = "http://";
		...
		bool crosschain_interface_doge::valid_config()
		{
			if (_config.contains("ip") && _config.contains("port"))
				return true;
			return false;
		}
		bool crosschain_interface_doge::unlock_wallet(std::string wallet_name, std::string wallet_passprase, uint32_t duration)
		{
			return false;
		}

		bool crosschain_interface_doge::open_wallet(std::string wallet_name)
		{
			return false;
		}

		void crosschain_interface_doge::close_wallet()
		{
			
		}

		std::vector<std::string> crosschain_interface_doge::wallet_list()
		{
			return std::vector<std::string>();
		}

		bool graphene::crosschain::crosschain_interface_doge::create_wallet(std::string wallet_name, std::string wallet_passprase)
		{
			return false;
		}

		std::string crosschain_interface_doge::create_normal_account(std::string account_name, const fc::optional<fc::ecc::private_key> key/*=fc::optional<fc::ecc::private_key>()*/)
		{
			...
			//	\"params\" : {\"chainId\":\"doge\" ,\"addr\": \"" << ptr->get_address() << "\"}}";
		...
		std::map<std::string,std::string> crosschain_interface_doge::create_multi_sig_account(std::string account_name, std::vector<std::string> addresses, uint32_t nrequired)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
				\"method\" : \"Zchain.Multisig.Create\" ,\
				\"params\" : {\"chainId\":\"doge\" ,\"amount\": " << nrequired <<",\""<<"addrs\":[";
		...
		std::vector<graphene::crosschain::hd_trx> crosschain_interface_doge::deposit_transaction_query(std::string user_account, uint32_t from_block, uint32_t limit)
		{
			return std::vector<graphene::crosschain::hd_trx>();
		}

		fc::variant_object crosschain_interface_doge::transaction_query(vector<std::string>& trx_ids)
		{
			try {
				std::ostringstream req_body;
				req_body << "{ \"jsonrpc\": \"2.0\", \
		...
		fc::variant_object crosschain_interface_doge::transaction_query(std::string trx_id)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
				\"method\" : \"Zchain.Trans.queryTrans\" ,\
				\"params\" : {\"chainId\":\"doge\" ,\"trxid\": \"" << trx_id << "\"}}";
			std::cout << req_body.str() << std::endl;
			for (const auto& q : _trxs_queue)
			{
				if (get<0>(q) == req_body.str())
		...
		fc::variant_object crosschain_interface_doge::transfer(const std::string &from_account, const std::string &to_account, uint64_t amount, const std::string &symbol, const std::string &memo, bool broadcast /*= true*/)
		{
			return fc::variant_object();
		}


		fc::variant_object crosschain_interface_doge::create_multisig_transaction(const std::string &from_account, const std::map<std::string, std::string> dest_info, const std::string &symbol, const std::string &memo)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
				\"method\" : \"Zchain.Trans.createTrx\" ,\
				\"params\" : {\"chainId\":\"doge\" ,\"from_addr\": \"" << from_account << "\",\"dest_info\":{";// << to_account << "\",\"amount\":" << amount << "}}";
		...
		fc::variant_object crosschain_interface_doge::create_multisig_transaction(std::string &from_account, std::string &to_account, const std::string& amount, std::string &symbol, std::string &memo, bool broadcast)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
				\"method\" : \"Zchain.Trans.createTrx\" ,\
				\"params\" : {\"chainId\":\"doge\" ,\"from_addr\": \"" << from_account << "\",\"to_addr\":\""<<to_account <<"\",\"amount\":" <<amount <<"}}";
			std::cout << req_body.str() << std::endl;
			fc::http::connection_sync conn;
			connect_midware(conn);
			//conn.connect_to(fc::ip::endpoint(fc::ip::address(_config["ip"].as_string()), _config["port"].as_uint64()));
		...
		std::string crosschain_interface_doge::sign_multisig_transaction(fc::variant_object trx, graphene::privatekey_management::crosschain_privatekey_base*& sign_key,const std::string& redeemScript,bool broadcast /*= true*/)
		{
			...
			req_body << "{ \"jsonrpc\": \"2.0\", \
				...
				\"params\" : {\"chainId\":\"doge\" ,\"addr\": \"" << sign_account << "\",\"trx_hex\":\"" << trx["hex"].as_string() << "\","<<"\"redeemScript"<<"\":"<<"\""<<redeemScript<<"\"}}";
			std::cout << req_body.str() << std::endl;
			fc::http::connection_sync conn;
			conn.connect_to(fc::ip::endpoint(fc::ip::address(_config["ip"].as_string()), _config["port"].as_uint64()));
			auto response = conn.request(_rpc_method, _rpc_url, req_body.str(), _rpc_headers);
		...
		fc::variant_object crosschain_interface_doge::merge_multisig_transaction(fc::variant_object &trx, std::vector<std::string> signatures)
		{
			graphene::privatekey_management::doge_privatekey btk;
			return btk.combine_trxs(signatures);
			/*std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
				\"id\" : \"45\", \
				\"method\" : \"Zchain.Trans.CombineTrx\" ,\
				\"params\" : {\"chainId\":\"doge\" ,\"transactions\": [";
		...
		bool crosschain_interface_doge::validate_link_trx(const hd_trx &trx)
		{
		...
		bool crosschain_interface_doge::validate_link_trx(const std::vector<hd_trx> &trx)
		{
			return false;
		}

		bool crosschain_interface_doge::validate_other_trx(const fc::variant_object &trx)
		{
			return true;
		}
		bool crosschain_interface_doge::validate_transaction( const std::string& addr,const std::string& redeemscript,const std::string& sig)
		{
			try {
				graphene::privatekey_management::doge_privatekey ltk;
				return ltk.validate_transaction(addr,redeemscript,sig);
			}FC_CAPTURE_AND_RETHROW((addr)(redeemscript)(sig));
		}


		bool crosschain_interface_doge::validate_address(const std::string& addr)
		{
		...
		bool crosschain_interface_doge::validate_signature(const std::string &account, const std::string &content, const std::string &signature)
		{
			graphene::privatekey_management::doge_privatekey ltk;
			...
			req_body << "{ \"jsonrpc\": \"2.0\", \
				...
				\"params\" : {\"chainId\":\"doge\" ,\"addr\": " << "\"" << account << "\"," << "\"message\":" << "\""  \
		...
		bool crosschain_interface_doge::create_signature(graphene::privatekey_management::crosschain_privatekey_base*& sign_key, const std::string &content, std::string &signature)
		{
			...
			req_body << "{ \"jsonrpc\": \"2.0\", \
				...
				\"params\" : {\"chainId\":\"doge\" ,\"addr\": " <<"\""<<account<<"\"," <<"\"message\":"<<"\""<<content<<"\"" <<"}}";
			fc::http::connection_sync conn;
			conn.connect_to(fc::ip::endpoint(fc::ip::address(_config["ip"].as_string()), _config["port"].as_uint64()));
			auto response = conn.request(_rpc_method, _rpc_url, req_body.str(), _rpc_headers);
			if (response.status == fc::http::reply::OK)
		...
		std::string crosschain_interface_doge::get_from_address(const fc::variant_object& trx)
		{
			try {
		...
		crosschain_trx crosschain_interface_doge::turn_trxs(const fc::variant_object & trx)
		{
			hd_trx hdtx;
		...
		graphene::crosschain::hd_trx crosschain_interface_doge::turn_trx(const fc::variant_object & trx)
		{
			hd_trx hdtx;
			try {
				hdtx.asset_symbol = chain_type;
		...
		void crosschain_interface_doge::broadcast_transaction(const fc::variant_object &trx)
		{
			try {
				std::ostringstream req_body;
				req_body << "{ \"jsonrpc\": \"2.0\", \
				...
				\"params\" : {\"chainId\":\"doge\" ,\"trx\": " << "\"" << trx["hex"].as_string() <<"\"" << "}}";
				fc::http::connection_sync conn;
				connect_midware(conn);
				//conn.connect_to(fc::ip::endpoint(fc::ip::address(_config["ip"].as_string()), _config["port"].as_uint64()));
				auto response = conn.request(_rpc_method, _rpc_url, req_body.str(), _rpc_headers);
		...
		std::vector<fc::variant_object> crosschain_interface_doge::query_account_balance(const std::string &account)
		{
			return std::vector<fc::variant_object>();
		}

		std::vector<fc::variant_object> crosschain_interface_doge::transaction_history(std::string symbol,const std::string &user_account, uint32_t start_block, uint32_t limit, uint32_t& end_block_num)
		{
			std::vector<fc::variant_object> return_value;
			std::string local_symbol = "doge";
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
		...
		std::string crosschain_interface_doge::export_private_key(std::string &account, std::string &encrypt_passprase)
		{
			std::ostringstream req_body;
		...
		std::string crosschain_interface_doge::import_private_key(std::string &account, std::string &encrypt_passprase)
		{
			return std::string();
		}

		std::string crosschain_interface_doge::backup_wallet(std::string &wallet_name, std::string &encrypt_passprase)
		{
			return std::string();
		}
...
// 		}
		...
		std::string crosschain_interface_doge::recover_wallet(std::string &wallet_name, std::string &encrypt_passprase)
		{
			return std::string();
		}
		std::vector<fc::variant_object> crosschain_interface_doge::transaction_history_all(std::vector<fc::mutable_variant_object> mul_param_obj) {
			std::vector<fc::variant_object> return_value;
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
