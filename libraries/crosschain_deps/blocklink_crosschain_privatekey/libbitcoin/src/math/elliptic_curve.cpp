    ...
    const ec_secret& secret)
{
    ...
    return parse(context, pubkey, in_out) &&
		libbitcoin_secp256k1_ec_pubkey_tweak_add(context, &pubkey, secret.data()) == 1 &&
    ...
    const ec_secret& secret)
{
    secp256k1_pubkey pubkey;
    return parse(context, pubkey, in_out) &&
		libbitcoin_secp256k1_ec_pubkey_tweak_mul(context, &pubkey, secret.data()) == 1 &&
        serialize(context, in_out, pubkey);
}
...
{
    secp256k1_pubkey pubkey;
    return libbitcoin_secp256k1_ec_pubkey_create(context, &pubkey, secret.data()) == 1 &&
    ...
    const ec_signature& signature)
{
    ...
    return libbitcoin_secp256k1_ecdsa_verify(context, &normal, hash.data(), &point) == 1;
}
...
{
    const auto context = verification.context();
    return libbitcoin_secp256k1_ec_privkey_tweak_add(context, left.data(),
        right.data()) == 1;
}
...
{
    const auto context = verification.context();
    return libbitcoin_secp256k1_ec_privkey_tweak_mul(context, left.data(),
        right.data()) == 1;
}
...
{
    const auto context = verification.context();
    return libbitcoin_secp256k1_ec_seckey_verify(context, secret.data()) == 1;
}
...
bool sign(ec_signature& out, const ec_secret& secret, const hash_digest& hash)
{
    ...
    if (libbitcoin_secp256k1_ecdsa_sign(context, &signature, hash.data(), secret.data(),
    ...
    const ec_signature& signature)
{
    ...
    return
		...
		libbitcoin_secp256k1_ecdsa_verify(context, &normal, hash.data(), &pubkey) == 1;
}
