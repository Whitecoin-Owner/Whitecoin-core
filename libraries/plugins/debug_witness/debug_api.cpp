...
namespace graphene { namespace debug_candidate {
...
class debug_api_impl
{
   public:
      ...
      std::shared_ptr< graphene::debug_candidate_plugin::debug_candidate_plugin > get_plugin();
...
void debug_api_impl::debug_generate_blocks( const std::string& debug_key, uint32_t count )
{
   ...
   for( uint32_t i=0; i<count; i++ )
   {
      graphene::chain::candidate_id_type scheduled_candidate = db->get_scheduled_candidate( 1 );
      ...
      {
         ilog( "Modified key for candidate ${w}", ("w", scheduled_candidate) );
         fc::mutable_variant_object update;
         update("_action", "update")("id", scheduled_candidate)("signing_key", debug_public_key);
         db->debug_update( update );
      }
      db->generate_block( scheduled_time, scheduled_candidate, *debug_private_key, graphene::chain::database::skip_nothing );
   }
}
...
std::shared_ptr< graphene::debug_candidate_plugin::debug_candidate_plugin > debug_api_impl::get_plugin()
{
   return app.get_plugin< graphene::debug_candidate_plugin::debug_candidate_plugin >( "debug_candidate" );
}
