...
** The UOP_CLOSURE instruction must use the last available register,
** so that, if it invokes the GC, the GC knows which registers
** are in use at that time.
*/
static void codeclosure(LexState *ls, expdesc *v) {
    FuncState *fs = ls->fs->prev;
    init_exp(v, VRELOCABLE, luaK_codeABx(fs, UOP_CLOSURE, 0, fs->np - 1));
...
static void recfield(LexState *ls, struct ConsControl *cc) {
    ...
    expr(ls, &val);
    luaK_codeABC(fs, UOP_SETTABLE, cc->t->u.info, rkkey, luaK_exp2RK(fs, &val));
...
static void constructor(LexState *ls, expdesc *t) {
    ...
    int pc = luaK_codeABC(fs, UOP_NEWTABLE, 0, 0, 0);
...
static void funcargs(LexState *ls, expdesc *f, int line) {
    ...
    init_exp(f, VCALL, luaK_codeABC(fs, UOP_CALL, base, nparams + 1, 2));
...
static void simpleexp(LexState *ls, expdesc *v) {
    ...
    case TK_DOTS: {  /* vararg */
        ...
        init_exp(v, VVARARG, luaK_codeABC(fs, UOP_VARARG, 0, 1, 0));
        break;
    }
    case '{': {  /* constructor */
        constructor(ls, v);
...
static void check_conflict(LexState *ls, struct LHS_assign *lh, expdesc *v) {
    ...
    if (conflict) {
        ...
        OpCode op = (v->k == VLOCAL) ? UOP_MOVE : UOP_GETUPVAL;
...
static void gotostat(LexState *ls, int pc) {
	...
	*/
    //const char *msg = luaO_pushfstring(ls->L, "uvm lua not support `goto` symbol");
    //ls->L->force_stopping = true;
    //uvm::lua::api::throw_exception(ls->L, UVM_API_PARSER_ERROR, msg);
    //semerror(ls, msg);
    //if (ls->L->force_stopping)
    //    return;
    //luaX_syntaxerror(ls, msg);
...
static void forbody(LexState *ls, int base, int line, int nvars, int isnum) {
    ...
    checknext(ls, TK_DO);
    prep = isnum ? luaK_codeAsBx(fs, UOP_FORPREP, base, NO_JUMP) : luaK_jump(fs);
    ...
    if (isnum)  /* numeric for? */
        endfor = luaK_codeAsBx(fs, UOP_FORLOOP, base, NO_JUMP);
    else {  /* generic for */
        luaK_codeABC(fs, UOP_TFORCALL, base, 0, nvars);
        luaK_fixline(fs, line);
        endfor = luaK_codeAsBx(fs, UOP_TFORLOOP, base + 2, NO_JUMP);
    }
    ...
    if (block_follow(ls, 1) || ls->t.token == ';')
        ...
        if (hasmultret(e.k)) {
            ...
            if (e.k == VCALL && nret == 1) {  /* tail call? */
                SET_OPCODE(getcode(fs, &e), UOP_TAILCALL);
