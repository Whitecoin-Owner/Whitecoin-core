...
namespace uvm
{
    namespace parser
    {
        ...
        void UvmTokenParser::_push_new_token(int type, size_t line)
        {
			...
			token.end_linenumber = token.linenumber + uvm::util::string_lines_count(token_str) - 1;
...
#define MAX_TOKEN_PARSER_CYCLES_COUNT 10086
        ...
        void UvmTokenParser::_read_long_string(std::string &code, bool is_string, int sep)
        {
            ...
            while (_parsing_pos < code.length())
            {
                if (_parsing_buff_len >= LUA_TOKEN_MAX_LENGTH - 1)
                {
					...
					{
                        global_uvm_chain_api->throw_exception(_L, UVM_API_COMPILE_ERROR, std::string("too long token").c_str());
						return;
					}
                }
                switch (_current_char(code))
                {
                case EOF_TOKEN_CHAR:
                    global_uvm_chain_api->throw_exception(_L, UVM_API_COMPILE_ERROR, "undefined long %s (starting at line %d)", what, line);
                    return;
                case ']':
                {
            ...
            while (_parsing_pos < code.length())
            {
                ...
                case '-':
                {
                    ...
                    if (_parsing_pos >= code.length() || (_parsing_pos < code.length() && code[_parsing_pos] != '-'))
                    {
						...
						token.end_linenumber = token.linenumber + uvm::util::string_lines_count(token.token) - 1;
        ...
        std::string UvmTokenParser::dump() const
        {
            ...
            for (auto it = _tokens.begin(); it != _tokens.end(); ++it)
            {
				...
				{
					ss << it->token;
					lasttoken_lines_count = uvm::util::string_lines_count(it->token);
				}
                }
            }
            return ss.str();
