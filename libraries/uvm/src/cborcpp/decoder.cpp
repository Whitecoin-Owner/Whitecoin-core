...
CborObjectP decoder::run() {
	...
	while (1) {
			...
			if (_in->has_bytes(1)) {
				...
				case 7: // float
					if (minorType == 20) {
						...
						_state = STATE_FLOAT_DATA;
						_currentLength = minorType;
					}
					else if (minorType == 24) {
						_state = STATE_FLOAT_SIZE;
						_currentLength = 1;
					}
					else if (minorType == 25) { // 2 byte
						_currentLength = 2;
						...
						_state = STATE_ERROR;
						throw cbor_decode_exception("invalid float type");
					}
					break;
				}
			}
		...
		else if (_state == STATE_FLOAT_SIZE) {
			if (_in->has_bytes(_currentLength)) {
				switch (_currentLength) {
				case 1:
					_currentLength = _in->get_byte();
		...
		else if (_state == STATE_FLOAT_DATA) {
			if (_in->has_bytes(_currentLength)) {
				std::vector<char> data(_currentLength);
				_in->get_bytes(data.data(), _currentLength);
				_state = STATE_TYPE;
