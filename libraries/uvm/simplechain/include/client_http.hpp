...
namespace SimpleWeb {
  ...
  public:
    ...
    class Response {
      ...
      Response(std::size_t max_response_streambuf_size) BOOST_NOEXCEPT : streambuf(max_response_streambuf_size), content(streambuf) {}
  ...
  protected:
    ...
    public:
      ...
      Connection(std::shared_ptr<ScopeRunner> handler_runner_, long timeout, Args &&... args) BOOST_NOEXCEPT
      ...
      void set_timeout(long seconds = 0) BOOST_NOEXCEPT {
        if(seconds == 0)
          seconds = timeout;
        if(seconds == 0) {
          timer = nullptr;
    ...
    class Session {
    public:
      Session(std::size_t max_response_streambuf_size, std::shared_ptr<Connection> connection_, std::unique_ptr<asio::streambuf> request_streambuf_) BOOST_NOEXCEPT
  ...
  public:
    ...
    virtual ~ClientBase() BOOST_NOEXCEPT {
      handler_runner->stop();
      stop();
    }
  ...
  protected:
    ...
    ClientBase(const std::string &host_port, unsigned short default_port) BOOST_NOEXCEPT : default_port(default_port), handler_runner(new ScopeRunner()) {
      auto parsed_host_port = parse_host_port(host_port, default_port);
      host = parsed_host_port.first;
      port = parsed_host_port.second;
    }

    std::shared_ptr<Connection> get_connection() BOOST_NOEXCEPT {
      std::shared_ptr<Connection> connection;
      std::unique_lock<std::mutex> lock(connections_mutex);
    ...
    std::pair<std::string, unsigned short> parse_host_port(const std::string &host_port, unsigned short default_port) const BOOST_NOEXCEPT {
    ...
    virtual std::shared_ptr<Connection> create_connection() BOOST_NOEXCEPT = 0;
  ...
  public:
    Client(const std::string &server_port_path) BOOST_NOEXCEPT : ClientBase<HTTP>::ClientBase(server_port_path, 80) {}

  protected:
    std::shared_ptr<Connection> create_connection() BOOST_NOEXCEPT override {
