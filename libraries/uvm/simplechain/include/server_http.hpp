...
namespace SimpleWeb {
  ...
  public:
    class Response : public std::enable_shared_from_this<Response>, public std::ostream {
      ...
      Response(std::shared_ptr<Session> session_, long timeout_content) BOOST_NOEXCEPT : std::ostream(nullptr), session(std::move(session_)), timeout_content(timeout_content), strand(session->connection->socket->get_io_service()) {
        rdbuf(streambuf.get());
      }
      ...
      void send_on_delete(const std::function<void(const error_code &)> &callback = nullptr) BOOST_NOEXCEPT {
    ...
    public:
      ...
      void send(const std::function<void(const error_code &)> &callback = nullptr) BOOST_NOEXCEPT {
        session->connection->set_timeout(timeout_content);
    ...
    public:
      ...
      std::string string() BOOST_NOEXCEPT {
        try {
          std::string str;
    ...
    private:
      ...
      Content(asio::streambuf &streambuf) BOOST_NOEXCEPT : std::istream(&streambuf), streambuf(streambuf) {}
    };
    ...
    class Request {
      ...
      Request(std::size_t max_request_streambuf_size, std::shared_ptr<asio::ip::tcp::endpoint> remote_endpoint_) BOOST_NOEXCEPT
    ...
    public:
      ...
      std::string remote_endpoint_address() BOOST_NOEXCEPT {
        try {
      ...
      unsigned short remote_endpoint_port() BOOST_NOEXCEPT {
        return remote_endpoint->port();
      }

      /// Returns query keys with percent-decoded values.
      CaseInsensitiveMultimap parse_query_string() BOOST_NOEXCEPT {
  ...
  protected:
    ...
    public:
      ...
      Connection(std::shared_ptr<ScopeRunner> handler_runner_, Args &&... args) BOOST_NOEXCEPT : handler_runner(std::move(handler_runner_)), socket(new socket_type(std::forward<Args>(args)...)) {}
      ...
      void set_timeout(long seconds) BOOST_NOEXCEPT {
        if(seconds == 0) {
          timer = nullptr;
          return;
        }
      ...
      void cancel_timeout() BOOST_NOEXCEPT {
        if(timer) {
          error_code ec;
          timer->cancel(ec);
        }
    ...
    class Session {
    public:
      Session(std::size_t max_request_streambuf_size, std::shared_ptr<Connection> connection_) BOOST_NOEXCEPT : connection(std::move(connection_)) {
  ...
  public:
    class Config {
      ...
      Config(unsigned short port) BOOST_NOEXCEPT : port(port) {}
  ...
  private:
    ...
    public:
      ...
      bool operator<(const regex_orderable &rhs) const BOOST_NOEXCEPT {
        return str < rhs.str;
      }
    };
  ...
  public:
    ...
    virtual ~ServerBase() BOOST_NOEXCEPT {
      handler_runner->stop();
      stop();
    }
  ...
  protected:
    ...
    ServerBase(unsigned short port) BOOST_NOEXCEPT : config(port), connections(new std::unordered_set<Connection *>()), connections_mutex(new std::mutex()), handler_runner(new ScopeRunner()) {}

    virtual void after_bind() {}
    virtual void accept() = 0;

    template <typename... Args>
    std::shared_ptr<Connection> create_connection(Args &&... args) BOOST_NOEXCEPT {
  ...
  public:
    Server() BOOST_NOEXCEPT : ServerBase<HTTP>::ServerBase(80) {}
