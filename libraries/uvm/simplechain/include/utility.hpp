...
namespace SimpleWeb {
  inline bool case_insensitive_equal(const std::string &str1, const std::string &str2) BOOST_NOEXCEPT {
  ...
  public:
    bool operator()(const std::string &str1, const std::string &str2) const BOOST_NOEXCEPT {
  ...
  public:
    std::size_t operator()(const std::string &str) const BOOST_NOEXCEPT {
      std::size_t h = 0;
      std::hash<int> hash;
      for(auto c : str)
  ...
  public:
    ...
    static std::string encode(const std::string &value) BOOST_NOEXCEPT {
    ...
    static std::string decode(const std::string &value) BOOST_NOEXCEPT {
      std::string result;
  ...
  public:
    ...
    static std::string create(const CaseInsensitiveMultimap &fields) BOOST_NOEXCEPT {
      std::string result;
    ...
    static CaseInsensitiveMultimap parse(const std::string &query_string) BOOST_NOEXCEPT {
  ...
  public:
    /// Parse header fields
    static CaseInsensitiveMultimap parse(std::istream &stream) BOOST_NOEXCEPT {
      CaseInsensitiveMultimap result;
      std::string line;
  ...
  public:
    ...
    static bool parse(std::istream &stream, std::string &method, std::string &path, std::string &query_string, std::string &version, CaseInsensitiveMultimap &header) BOOST_NOEXCEPT {
      std::string line;
  ...
  public:
    ...
    static bool parse(std::istream &stream, std::string &version, std::string &status_code, CaseInsensitiveMultimap &header) BOOST_NOEXCEPT {
      std::string line;
...
#include <emmintrin.h>
namespace SimpleWeb {
  inline void spin_loop_pause() BOOST_NOEXCEPT { _mm_pause(); }
} // namespace SimpleWeb
...
#else
namespace SimpleWeb {
  inline void spin_loop_pause() BOOST_NOEXCEPT {}
} // namespace SimpleWeb
#endif
...
namespace SimpleWeb {
  ...
  public:
    class SharedLock {
      ...
      std::atomic<long> &count;
      SharedLock(std::atomic<long> &count) BOOST_NOEXCEPT : count(count) {}
      SharedLock &operator=(const SharedLock &) = delete;
      SharedLock(const SharedLock &) = delete;

    public:
      ~SharedLock() BOOST_NOEXCEPT {
        count.fetch_sub(1);
      }
    };

    ScopeRunner() BOOST_NOEXCEPT : count(0) {}

    /// Returns nullptr if scope should be exited, or a shared lock otherwise
    std::unique_ptr<SharedLock> continue_lock() BOOST_NOEXCEPT {
      long expected = count;
