		...
		void database::adjust_crosschain_transaction(transaction_id_type relate_transaction_id,
			...
			try {
				if (op_type == operation::tag<crosschain_withdraw_evaluate::operation_type>::value) {
					...
					{
						trx_st = withdraw_eth_wallfacer_need_sign;
					}
					else {
...
// 					FC_ASSERT(trx_itr == trx_db.end(), "This sign tex is exist");
					...
					std::for_each(combine_state_range.first, combine_state_range.second, [&](const crosschain_trx_object& sign_tx) {
						auto sign_iter = tx_db_objs.find(sign_tx.transaction_id);
						count++;
						if (sign_iter->trx_state == withdraw_combine_trx_create || sign_iter->trx_state == withdraw_eth_wallfacer_sign) {
							modify(*sign_iter, [&](crosschain_trx_object& obj) {
								obj.trx_state = withdraw_transaction_confirm;
								obj.block_num = head_block_num();
							});
				...
				else if (op_type == operation::tag<eths_wallfacer_sign_final_operation>::value) {
					auto & tx_db_objs = get_index_type<crosschain_trx_index>().indices().get<by_transaction_id>();
					...
					auto op = real_transaction.operations[0];
					auto eth_signed_op = op.get<eths_wallfacer_sign_final_operation>();
					...
					modify(*tx_combine_sign_iter, [&](crosschain_trx_object& obj) {
						obj.trx_state = withdraw_eth_wallfacer_sign;
					...
					std::for_each(sign_range.first, sign_range.second, [&](const crosschain_trx_object& sign_tx) {
						auto sign_iter = tx_db_objs.find(sign_tx.transaction_id);
						count++;
						if (sign_iter->trx_state == withdraw_eth_wallfacer_need_sign) {
							modify(*sign_iter, [&](crosschain_trx_object& obj) {
								obj.trx_state = withdraw_eth_wallfacer_sign;
								obj.crosschain_trx_id = signed_trxid;
								obj.block_num = head_block_num();
				...
				else if (op_type == operation::tag<eths_wallfacer_change_signer_operation>::value) {
					auto & tx_db_objs = get_index_type<crosschain_trx_index>().indices().get<by_transaction_id>();
					auto tx_combine_sign_iter = tx_db_objs.find(relate_transaction_id);
					auto tx_without_sign_iter = tx_db_objs.find(tx_combine_sign_iter->relate_transaction_id);
					auto op = real_transaction.operations[0];
					auto eth_signed_op = op.get<eths_wallfacer_change_signer_operation>();
					std::string signed_trxid = eth_signed_op.signed_crosschain_trx_id;
					if (eth_signed_op.signed_crosschain_trx_id.find('|') != eth_signed_op.signed_crosschain_trx_id.npos) {
						auto pos = eth_signed_op.signed_crosschain_trx_id.find('|');
						signed_trxid = eth_signed_op.signed_crosschain_trx_id.substr(pos + 1);
		...
		void database::create_result_transaction(miner_id_type miner, fc::ecc::private_key pk) {
			try {
				...
				auto coldhot_check1 = get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_state>().equal_range(coldhot_trx_state::coldhot_eth_wallfacer_need_sign);
				auto coldhot_check2 = get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_state>().equal_range(coldhot_trx_state::coldhot_eth_wallfacer_sign);
				...
				auto check_point_3 = get_index_type<crosschain_trx_index>().indices().get<by_transaction_stata>().equal_range(withdraw_eth_wallfacer_need_sign);
				auto check_point_4 = get_index_type<crosschain_trx_index>().indices().get<by_transaction_stata>().equal_range(withdraw_eth_wallfacer_sign);
				...
				for (auto & acquired_trxs : acquired_crosschain_trx) {
					...
					for (auto acquired_trx : acquired_trxs.second) {
						if ((acquired_trxs.first.find("ETH") != acquired_trxs.first.npos) || (acquired_trxs.first.find("ERC") != acquired_trxs.first.npos)) {
							...
							{
								if (cold_multi_obj.state == sol_create_wallfacer_signed 
									&& cold_multi_obj.op_type == operation::tag<eths_multi_sol_wallfacer_sign_operation>::value
							...
							{
								if (hot_multi_obj.state == sol_create_wallfacer_signed 
									&& hot_multi_obj.op_type == operation::tag<eths_multi_sol_wallfacer_sign_operation>::value
		...
		void database::combine_sign_transaction(miner_id_type miner, fc::ecc::private_key pk) {
			try {
				...
				auto wallfacer_members = get_wallfacer_members();
				for (auto & trxs : uncombine_trxs_counts) {
					//TODO : Use macro instead this magic number
					if (trxs.second.size() < (wallfacer_members.size()*2/3+1)) {
						continue;
					}
					map<account_id_type,string> combine_signature;
					account_id_type sign_wallfacer;
					auto& trx_db = get_index_type<crosschain_trx_index>().indices().get<by_transaction_id>();
					const auto& wallfacer_db = get_index_type<wallfacer_member_index>().indices().get<by_id>();
					...
					for (auto & sign_trx_id : trxs.second) {
						...
						combine_signature[get(with_sign_op.sign_wallfacer).wallfacer_member_account]= with_sign_op.ccw_trx_signature;
						if (sign_wallfacer == account_id_type()) {
							auto wallfacer_iter = wallfacer_db.find(with_sign_op.sign_wallfacer);
							FC_ASSERT(wallfacer_iter != wallfacer_db.end());
							if (wallfacer_iter->wallfacer_type == PERMANENT) {
								sign_wallfacer = wallfacer_iter->wallfacer_member_account;
							}
					...
					vector<string> wallfacer_signed;
					for (const auto& iter : combine_signature)
					{
						wallfacer_signed.push_back(iter.second);
						if (wallfacer_signed.size() == (wallfacer_members.size() * 2 / 3 + 1))
							break;
					}
					if (with_sign_op.asset_symbol == "ETH" || with_sign_op.asset_symbol.find("ERC") != with_sign_op.asset_symbol.npos)
					{
						try {
							auto wallfacer_account_id =sign_wallfacer;
							multisig_address_object wallfacer_multi_obj;
							const auto& multisig_addr_by_wallfacer = get_index_type<multisig_address_index>().indices().get<by_account_chain_type>();
							const auto iter_range = multisig_addr_by_wallfacer.equal_range(boost::make_tuple(with_sign_op.asset_symbol, wallfacer_account_id));
							...
							trx_op.cross_chain_trx = hdl->merge_multisig_transaction(temp_obj, wallfacer_signed);
						}	FC_CAPTURE_AND_LOG((0));					
					}
					else {
					trx_op.cross_chain_trx = hdl->merge_multisig_transaction(with_sign_op.withdraw_source_trx, wallfacer_signed);
					}
