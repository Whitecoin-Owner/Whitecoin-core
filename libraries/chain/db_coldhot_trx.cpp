...
namespace graphene {
	namespace chain {
		void database::adjust_coldhot_transaction(transaction_id_type relate_trx_id, transaction_id_type current_trx_id, signed_transaction current_trx, uint64_t op_type) {
			try {
				if (op_type == operation::tag<coldhot_transfer_operation>::value) {
					...
					{
						tx_st = coldhot_eth_wallfacer_need_sign;
					}
					else {
					...
					for (auto combine_state_tx : boost::make_iterator_range(combine_state_tx_range.first, combine_state_tx_range.second)) {
						if (combine_state_tx.curret_trx_state != coldhot_combine_trx_create && combine_state_tx.curret_trx_state != coldhot_eth_wallfacer_sign) {
							continue;
						}
				...
				else if (op_type == operation::tag<eths_coldhot_wallfacer_sign_final_operation>::value){
				...
				auto op = current_trx.operations[0];
				auto eth_signed_op = op.get<eths_coldhot_wallfacer_sign_final_operation>();
				...
				modify(*tx_combine_sign_iter, [&](coldhot_transfer_object& obj) {
					obj.curret_trx_state = coldhot_eth_wallfacer_sign;
				...
				std::for_each(sign_range.first, sign_range.second, [&](const coldhot_transfer_object& sign_tx) {
					...
					if (sign_iter->curret_trx_state == coldhot_eth_wallfacer_need_sign) {
				...
				else if (op_type == operation::tag<eths_wallfacer_coldhot_change_signer_operation>::value) {
					auto & tx_db_objs = get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_id>();
					auto tx_combine_sign_iter = tx_db_objs.find(relate_trx_id);
					auto tx_without_sign_iter = tx_db_objs.find(tx_combine_sign_iter->relate_trx_id);
					auto op = current_trx.operations[0];
					auto eth_signed_op = op.get<eths_wallfacer_coldhot_change_signer_operation>();
					std::string signed_trxid = eth_signed_op.signed_crosschain_trx_id;
					if (eth_signed_op.signed_crosschain_trx_id.find('|') != eth_signed_op.signed_crosschain_trx_id.npos) {
						auto pos = eth_signed_op.signed_crosschain_trx_id.find('|');
						signed_trxid = eth_signed_op.signed_crosschain_trx_id.substr(pos + 1);
		...
		void database::create_coldhot_transfer_trx(miner_id_type miner, fc::ecc::private_key pk) {
			...
			auto transfer_check1 = get_index_type<crosschain_trx_index>().indices().get<by_transaction_stata>().equal_range(withdraw_eth_wallfacer_need_sign);
			auto transfer_check2 = get_index_type<crosschain_trx_index>().indices().get<by_transaction_stata>().equal_range(withdraw_eth_wallfacer_sign);
			...
			auto check_point_3 = get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_state>().equal_range(coldhot_trx_state::coldhot_eth_wallfacer_need_sign);
			auto check_point_4 = get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_state>().equal_range(coldhot_trx_state::coldhot_eth_wallfacer_sign);
			...
			for (auto & trxs : uncombine_trxs) {
				try {
					...
					for (auto & sign_trx_id : trxs.second) {
						...
						combine_signature[get(coldhot_sign_op.sign_wallfacer).wallfacer_member_account] = coldhot_sign_op.coldhot_transfer_sign;
						if (sign_wallfacer == account_id_type()) {
							auto wallfacer_iter = wallfacer_db.find(coldhot_sign_op.sign_wallfacer);
					...
					vector<string> wallfacer_signs;
					for (const auto& iter : combine_signature)
					{
						wallfacer_signs.push_back(iter.second);
						if (wallfacer_signs.size() == (coldhot_op.withdraw_account_count * 2 / 3 + 1))
							break;
					}
					if (coldhot_op.asset_symbol == "ETH" || coldhot_op.asset_symbol.find("ERC") != coldhot_op.asset_symbol.npos)
					{
						try {
							...
							const auto iter_range = multisig_addr_by_wallfacer.equal_range(boost::make_tuple(coldhot_op.asset_symbol, wallfacer_account_id));
							...
							trx_op.coldhot_trx_original_chain = crosschain_plugin->merge_multisig_transaction(temp_obj, wallfacer_signs);
						}	FC_CAPTURE_AND_LOG((0));
					}
					else {
						trx_op.coldhot_trx_original_chain = crosschain_plugin->merge_multisig_transaction(coldhot_op.coldhot_trx_original_chain, wallfacer_signs);
					}
