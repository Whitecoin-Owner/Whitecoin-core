...
struct get_impacted_account_visitor
{
   ...
   void operator()(const wallfacer_lock_balance_operation& op) {}
   void operator()(const pay_back_operation& op) {}
   void operator()(const bonus_operation& op) {}
   void operator()(const set_wallfacer_lockbalance_operation& op) {}
   void operator()(const wallfacer_foreclose_balance_operation& op) {}
   ...
   void operator() (const eths_wallfacer_change_signer_operation& op){}
   ...
   void operator()( const wallfacer_member_create_operation& op )
   {
      _impacted.insert( op.wallfacer_member_account );
   }
   void operator()( const wallfacer_member_update_operation& op ){}
   void operator()( const committee_member_update_global_parameters_operation& op ) {}
   void operator()(const wallfacer_member_resign_operation& op)
   {
       _impacted.insert(op.wallfacer_member_account);
   }
   ...
   void operator()(const eths_multi_sol_wallfacer_sign_operation & op) {}
   void operator()(const eth_multi_account_create_record_operation & op) {}
   void operator()(const eths_coldhot_wallfacer_sign_final_operation & op) {}
   ...
   if( obj->id.space() == protocol_ids )
   {
      switch( (object_type)obj->id.type() )
      {
        ...
        case account_object_type:{
           ...
           const auto& aobj = dynamic_cast<const wallfacer_member_object*>(obj);
           assert( aobj != nullptr );
