		...
		void_result crosschain_withdraw_result_evaluate::do_evaluate(const crosschain_withdraw_result_operation& o) {
			try {
				...
				if(current_blockNum > CROSSCHAIN_RECORD_EVALUATE_1000000)
				{
					if (o.cross_chain_trx.asset_symbol == "ETH" || o.cross_chain_trx.asset_symbol.find("ERC") != o.cross_chain_trx.asset_symbol.npos)
					{
						FC_ASSERT(combine_trx_iter->trx_state == withdraw_eth_wallfacer_sign);
					}
				}
		...
		void_result crosschain_withdraw_with_sign_evaluate::do_evaluate(const crosschain_withdraw_with_sign_operation& o) {
			...
			try {
				...
				for (auto sign_obj : boost::make_iterator_range(sign_range.first, sign_range.second)) {
					...
					std::string sig = fc::variant(sign_op.sign_wallfacer).as_string();
					signs.insert(sig);
				}
				...
				auto sign_iter = signs.find(fc::variant(o.sign_wallfacer).as_string());
				...
				FC_ASSERT(hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_hot, multisig_account_obj.redeemScript_hot, o.ccw_trx_signature) || hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_cold, multisig_account_obj.redeemScript_cold, o.ccw_trx_signature));
				}
				return void_result();
			}FC_CAPTURE_AND_RETHROW((o));
