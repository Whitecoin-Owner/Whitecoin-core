	...
	namespace chain {
		...
		void_result coldhot_transfer_with_sign_evaluate::do_evaluate(const coldhot_transfer_with_sign_operation& o) {
			try {
				...
				for (auto coldhot_sign_tx : boost::make_iterator_range(range.first, range.second)) {
					...
					if (sign_op.sign_wallfacer == o.sign_wallfacer) {
						isSigned = true;
						break;
					}
				}
				...
				{
					from_account = hdl->get_from_address(o.coldhot_trx_original_chain);
				}
				else {
					hd_trxs = hdl->turn_trxs(fc::variant_object("turn_without_eth_sign", o.coldhot_trx_original_chain));
					FC_ASSERT(hd_trxs.trxs.size() >= 1);
					auto crosschain_tx = hd_trxs.trxs.begin()->second;
					from_account = crosschain_tx.from_account;
				}
				
				vector<multisig_address_object>  wallfacer_pubks = db().get_multi_account_wallfacer(from_account, o.asset_symbol);
				FC_ASSERT(wallfacer_pubks.size() > 0);
				auto& acc_idx = db().get_index_type<account_index>().indices().get<by_address>();
				auto acc_itr = acc_idx.find(o.wallfacer_address);
				FC_ASSERT(acc_itr != acc_idx.end());
				int index = 0;
				for (; index < wallfacer_pubks.size(); index++)
				{
					if (wallfacer_pubks[index].wallfacer_account == acc_itr->get_id())
						break;
				}
				FC_ASSERT(index < wallfacer_pubks.size());
				auto multisig_account_obj = db().get(wallfacer_pubks[index].multisig_account_pair_object_id);
				if (o.asset_symbol == "BCH")
				{
					...
					FC_ASSERT(hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_hot,
						multisig_account_obj.redeemScript_hot,
						o.coldhot_transfer_sign + "|" + source_trx)
						|| hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_cold,
							multisig_account_obj.redeemScript_cold,
							o.coldhot_transfer_sign + "|" + source_trx));

				}
				else {
				FC_ASSERT(hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_hot, multisig_account_obj.redeemScript_hot, o.coldhot_transfer_sign) || hdl->validate_transaction(wallfacer_pubks[index].new_pubkey_cold, multisig_account_obj.redeemScript_cold, o.coldhot_transfer_sign));
				}
				return void_result();
			}FC_CAPTURE_AND_RETHROW((o))
		...
		void_result coldhot_transfer_result_evaluate::do_evaluate(const coldhot_transfer_result_operation& o) {
			try {
				...
				if (o.coldhot_trx_original_chain.asset_symbol == "ETH" || o.coldhot_trx_original_chain.asset_symbol.find("ERC") != o.coldhot_trx_original_chain.asset_symbol.npos)
				{
					FC_ASSERT(combine_trx_iter->curret_trx_state == coldhot_eth_wallfacer_sign);
				}
				...
				if (deposit_to_link_trx != deposit_db.end()) {
					if (deposit_to_link_trx->acquired_transaction_state != acquired_trx_uncreate) {
						FC_ASSERT(false,"deposit to link transaction doesn`t exist");
					}
				}
				auto& manager = graphene::crosschain::crosschain_manager::get_instance();
		...
		void_result coldhot_pass_combine_trx_evaluate::do_evaluate(const coldhot_pass_combine_trx_operation& o) {
			try {
				...
				FC_ASSERT(current_blockNum >= PASS_XWC_BLOCK_NUM, "You cant use this function now");
				auto & coldhot_db = d.get_index_type<coldhot_transfer_index>().indices().get<by_current_trx_id>();
				auto coldhot_iter = coldhot_db.find(o.pass_transaction_id);
				FC_ASSERT(coldhot_iter != coldhot_db.end());
				FC_ASSERT(coldhot_iter->curret_trx_state == coldhot_combine_trx_create, "Coldhot transaction state error");
		...
		void_result coldhot_pass_combine_trx_evaluate::do_apply(const coldhot_pass_combine_trx_operation& o) {
			try {
				FC_ASSERT(trx_state->_trx != nullptr, "trx_state->_trx should not be nullptr");
				db().adjust_coldhot_transaction(o.pass_transaction_id, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<coldhot_pass_combine_trx_operation>::value));
				return void_result();
			}FC_CAPTURE_AND_RETHROW((o))
		}
		
		void coldhot_pass_combine_trx_evaluate::pay_fee() {
