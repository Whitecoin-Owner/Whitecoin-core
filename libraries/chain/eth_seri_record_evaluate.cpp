	...
	namespace chain {
		...
		void_result eth_series_multi_sol_create_evaluator::do_evaluate(const eth_series_multi_sol_create_operation& o) {
			try {
				...
				vector<pair<account_id_type, pair<string, string>>> eth_wallfacer_account_ids;
				...
				auto func = [&wallfacer_ids](account_id_type id)->bool {
					...
					{
						if (wallfacer.wallfacer_member_account == id)
							return true;
					}
					return false;
				};
				...
				{
					std::for_each(
						addr_range.first, addr_range.second, [&symbol_addrs_cold, &symbol_addrs_hot,&func, &eth_wallfacer_account_ids](const multisig_address_object& obj) {

						if (obj.multisig_account_pair_object_id == multisig_account_pair_id_type())
						{
							if (func(obj.wallfacer_account)) {
								symbol_addrs_cold.push_back(obj.new_pubkey_cold);
								symbol_addrs_hot.push_back(obj.new_pubkey_hot);
								auto account_pair = make_pair(obj.wallfacer_account, make_pair(obj.new_pubkey_hot, obj.new_pubkey_cold));
								eth_wallfacer_account_ids.push_back(account_pair);
							}
						}
					}
					);
					...
					std::for_each(
						addr_range.first, addr_range.second, [&symbol_addrs_cold, &symbol_addrs_hot, &eth_wallfacer_account_ids](const multisig_address_object& obj) {
						if (obj.multisig_account_pair_object_id == multisig_account_pair_id_type())
						{
							symbol_addrs_cold.push_back(obj.new_pubkey_cold);
							symbol_addrs_hot.push_back(obj.new_pubkey_hot);
				...
				const auto& wallfacer_dbs = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto sign_wallfacer_iter = wallfacer_dbs.find(o.wallfacer_to_sign);
				FC_ASSERT(sign_wallfacer_iter != wallfacer_dbs.end());
				FC_ASSERT(sign_wallfacer_iter->wallfacer_type == PERMANENT);
				FC_ASSERT(sign_wallfacer_iter->formal == true);
				...
				for (auto wallfacer_account_id : eth_wallfacer_account_ids) {
					if (wallfacer_account_id.first == sign_wallfacer_iter->wallfacer_member_account){
						temp_hot = ptr->get_address_by_pubkey(wallfacer_account_id.second.first);
						temp_cold = ptr->get_address_by_pubkey(wallfacer_account_id.second.second);
						break;
					}
				}
				FC_ASSERT(temp_hot != "", "wallfacer donst has hot address");
				FC_ASSERT(temp_cold != "", "wallfacer donst has cold address");
				FC_ASSERT(o.wallfacer_sign_hot_address == temp_hot);
				FC_ASSERT(o.wallfacer_sign_cold_address == temp_cold);

				//const auto& wallfacer_ids = db().get_global_properties().active_committee_members;
				FC_ASSERT(symbol_addrs_cold.size() == wallfacer_ids.size() && symbol_addrs_hot.size() == wallfacer_ids.size()&& eth_wallfacer_account_ids.size() == wallfacer_ids.size());
		...
		void_result eth_series_multi_sol_wallfacer_sign_evaluator::do_evaluate(const eths_multi_sol_wallfacer_sign_operation& o) {
			try {
				const auto& wallfacer_db = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto wallfacer_iter = wallfacer_db.find(o.wallfacer_to_sign);
				FC_ASSERT(wallfacer_iter != wallfacer_db.end(), "This Guard doesnt exist");
				const auto& accounts = db().get_index_type<account_index>().indices().get<by_id>();
				const auto acct = accounts.find(wallfacer_iter->wallfacer_member_account);
				FC_ASSERT(acct->addr == o.wallfacer_sign_address);
				...
				FC_ASSERT(multi_account_create_iter->state == sol_create_need_wallfacer_sign);
				if (trx_state->_trx == nullptr)
					return void_result();
				FC_ASSERT(trx_state->_trx->operations.size() == 1);
				...
				auto hot_trx = handle->turn_trx(fc::variant_object("get_without_sign", o.multi_hot_sol_wallfacer_sign));
				auto cold_trx = handle->turn_trx(fc::variant_object("get_without_sign", o.multi_cold_sol_wallfacer_sign));
				auto hot_trx_id = (handle->turn_trx(fc::variant_object("get_with_sign", o.multi_hot_sol_wallfacer_sign))).trx_id;
				auto cold_trx_id = (handle->turn_trx(fc::variant_object("get_with_sign", o.multi_cold_sol_wallfacer_sign))).trx_id;
		...
		object_id_type eth_series_multi_sol_wallfacer_sign_evaluator::do_apply(const eths_multi_sol_wallfacer_sign_operation& o) {
			FC_ASSERT(trx_state->_trx != nullptr);
			db().adjust_eths_multi_account_record(o.sol_without_sign_txid, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<eths_multi_sol_wallfacer_sign_operation>::value));
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return object_id_type();
			}
		    ...
		    handle->broadcast_transaction(fc::variant_object("trx", "0x" + o.multi_hot_sol_wallfacer_sign));
			handle->broadcast_transaction(fc::variant_object("trx", "0x" + o.multi_cold_sol_wallfacer_sign)); 		
			return object_id_type();
		}
		void eth_series_multi_sol_wallfacer_sign_evaluator::pay_fee() {

		}
		void_result eths_wallfacer_change_signer_evaluator::do_evaluate(const eths_wallfacer_change_signer_operation& o) {
			try {
				const auto& wallfacer_db = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto wallfacer_iter = wallfacer_db.find(o.wallfacer_to_sign);
				FC_ASSERT(wallfacer_iter != wallfacer_db.end(), "This Guard doesnt exist");
				...
				FC_ASSERT(trx_iter->trx_state == withdraw_eth_wallfacer_need_sign);
		...
		object_id_type eths_wallfacer_change_signer_evaluator::do_apply(const eths_wallfacer_change_signer_operation& o) {
			FC_ASSERT(trx_state->_trx != nullptr);
			db().adjust_crosschain_transaction(o.combine_trx_id, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<eths_wallfacer_change_signer_operation>::value), withdraw_eth_wallfacer_sign);
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return object_id_type();
			}
		...
		void_result eths_wallfacer_sign_final_evaluator::do_evaluate(const eths_wallfacer_sign_final_operation& o) {
			try{
				const auto& wallfacer_db = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto wallfacer_iter = wallfacer_db.find(o.wallfacer_to_sign);
				FC_ASSERT(wallfacer_iter != wallfacer_db.end(), "This Guard doesnt exist");
		...
		object_id_type eths_wallfacer_sign_final_evaluator::do_apply(const eths_wallfacer_sign_final_operation& o) {
			FC_ASSERT(trx_state->_trx != nullptr);
			db().adjust_crosschain_transaction(o.combine_trx_id, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<eths_wallfacer_sign_final_operation>::value), withdraw_eth_wallfacer_sign);
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return object_id_type();
			}
		...
		void_result eths_wallfacer_coldhot_change_signer_evaluator::do_evaluate(const eths_wallfacer_coldhot_change_signer_operation& o) {
			try {
				const auto& wallfacer_db = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto wallfacer_iter = wallfacer_db.find(o.wallfacer_to_sign);
				FC_ASSERT(wallfacer_iter != wallfacer_db.end(), "This Guard doesnt exist");
		...
		object_id_type eths_wallfacer_coldhot_change_signer_evaluator::do_apply(const eths_wallfacer_coldhot_change_signer_operation& o) {
			FC_ASSERT(trx_state->_trx != nullptr);
			db().adjust_coldhot_transaction(o.combine_trx_id, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<eths_wallfacer_coldhot_change_signer_operation>::value));
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return object_id_type();
			}
		...
		void eths_wallfacer_coldhot_change_signer_evaluator::pay_fee() {

		}
		void_result eths_coldhot_wallfacer_sign_final_evaluator::do_evaluate(const eths_coldhot_wallfacer_sign_final_operation& o) {
			try {
				const auto& wallfacer_db = db().get_index_type<wallfacer_member_index>().indices().get<by_id>();
				auto wallfacer_iter = wallfacer_db.find(o.wallfacer_to_sign);
				FC_ASSERT(wallfacer_iter != wallfacer_db.end(), "This Guard doesnt exist");
		...
		object_id_type eths_coldhot_wallfacer_sign_final_evaluator::do_apply(const eths_coldhot_wallfacer_sign_final_operation& o) {
			FC_ASSERT(trx_state->_trx != nullptr);
			db().adjust_coldhot_transaction(o.combine_trx_id, trx_state->_trx->id(), *(trx_state->_trx), uint64_t(operation::tag<eths_coldhot_wallfacer_sign_final_operation>::value));
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return object_id_type();
			}
		...
		void eths_coldhot_wallfacer_sign_final_evaluator::pay_fee() {

		}
		void eths_wallfacer_sign_final_evaluator::pay_fee() {

		}
		void eths_wallfacer_change_signer_evaluator::pay_fee() {
		...
		void_result eth_multi_account_create_record_evaluator::do_evaluate(const eth_multi_account_create_record_operation& o) {
			...
			FC_ASSERT(multi_account_create_sign_iter->state == sol_create_wallfacer_signed);
			...
			FC_ASSERT(multi_acc_without_sign_iter->state == sol_create_wallfacer_signed);
			...
			const auto create_multi_account_op = multi_account_create_sign_iter->object_transaction.operations[0].get<eths_multi_sol_wallfacer_sign_operation>();
			auto& manager = graphene::crosschain::crosschain_manager::get_instance();
			if (!manager.contain_crosschain_handles(o.chain_type)) {
				return void_result();
			}
			...
			{
				auto cold_hdtx = handle->turn_trx(fc::variant_object("get_with_sign", create_multi_account_op.multi_cold_sol_wallfacer_sign));
				FC_ASSERT(multi_account_create_sign_iter->cold_trx_success == false);
				FC_ASSERT(cold_hdtx.trx_id == o.eth_multi_account_trx.trx_id);
			}
			else {
				auto hot_hdtx = handle->turn_trx(fc::variant_object("get_with_sign", create_multi_account_op.multi_hot_sol_wallfacer_sign));
		...
		void_result wallfacer_change_acquire_trx_evaluator::do_evaluate(const wallfacer_change_acquire_trx_operation& o) {
			const database& d = db();
			...
			FC_ASSERT(sign_final_iter->trx_state == withdraw_eth_wallfacer_sign, "cross chain trx state error");
			...
			FC_ASSERT(op.which() == operation::tag<eths_wallfacer_sign_final_operation>::value, "operation type error");
			auto sign_final_op = op.get<eths_wallfacer_sign_final_operation>();
		...
		object_id_type wallfacer_change_acquire_trx_evaluator::do_apply(const wallfacer_change_acquire_trx_operation& o) {
			const auto& trx_db = db().get_index_type<crosschain_trx_index>().indices().get<by_transaction_id>();
			const auto sign_final_iter = trx_db.find(o.change_transaction_id);
			auto sign_final_op = sign_final_iter->real_transaction.operations[0].get<eths_wallfacer_sign_final_operation>();
			string crosschain_trx_id = sign_final_op.signed_crosschain_trx_id;
			if (sign_final_op.chain_type == "ETH" || sign_final_op.chain_type.find("ERC") != sign_final_op.chain_type.npos)
			{
				if (crosschain_trx_id.find('|') != crosschain_trx_id.npos)
		...
		void wallfacer_change_acquire_trx_evaluator::pay_fee() {
