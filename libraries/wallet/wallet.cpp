...
public:
   ...
   string derive_wif_key(string brain_key, int index, const string& symbol="XWC")
   {
	   ...
	   auto pos = symbol.find("|etwallfacer");
   ...
   crosschain_prkeys create_crosschain_symbol(const string& symbol, bool cold=false,bool use_brain_key=false)
   {
	   try {
		   ...
		   if (bGuard){
			   wif_key = fd->create_normal_account("wallfacer",key);
		   }
		   else {
   ...
   std::pair<asset, share_type> register_contract_testing(const string& caller_account_name, const string& contract_filepath)
   {
       try {
           ...
           contract_register_op.owner_addr = acc_caller.addr;
           contract_register_op.owner_pubkey = caller_pubkey;
   ...
   std::pair<asset, share_type> register_native_contract_testing(const string & caller_account_name, const string & native_contract_key)
   {
       try {
           ...
           n_contract_register_op.owner_addr = acc_caller.addr;
           n_contract_register_op.owner_pubkey = caller_pubkey;
   ...
   full_transaction create_contract_transfer_fee_proposal(const string& proposer, share_type fee_rate, int64_t expiration_time, bool broadcast = false)
   {
	   try
	   {
		   ...
		   auto wallfacer_obj = get_wallfacer_member(proposer);
		   auto wallfacer_id = wallfacer_obj.wallfacer_member_account;
		   auto wallfacer_account_obj = get_account(wallfacer_id);
		   op.wallfacer = wallfacer_account_obj.addr;
		   op.wallfacer_id = wallfacer_obj.id;
	   ...
	   bool broadcast = false)
   {
	   try {
		   ...
		   auto issuer_account = get_wallfacer_member(issuer);
		   op.issuer = issuer_account.wallfacer_member_account;
   ...
   { try {
	  ...
	  FC_ASSERT(!is_locked());
      wallfacer_member_create_operation wallfacer_member_create_op;
	  auto wallfacer_member_account = get_account(account);
	  FC_ASSERT(account_object().get_id() != wallfacer_member_account.get_id(),"account is not registered to the chain.");
	  wallfacer_member_create_op.wallfacer_member_account = wallfacer_member_account.get_id();
	  wallfacer_member_create_op.fee_pay_address = wallfacer_member_account.addr;
	  wallfacer_member_create_op.guarantee_id = get_guarantee_id();

	  signed_transaction tx;
	  tx.operations.push_back(wallfacer_member_create_op);
   ...
   full_transaction resign_wallfacer_member(string proposing_account, string account, int64_t expiration_time, bool broadcast)
   {
       try {
           //account should be register in the blockchian
           FC_ASSERT(!is_locked());
           wallfacer_member_resign_operation op;
           auto wallfacer_member_account = get_account_id(account);
           FC_ASSERT(account_object().get_id() != wallfacer_member_account, "account is not registered to the chain.");
           op.wallfacer_member_account = wallfacer_member_account;

           const chain_parameters& current_params = get_global_properties().parameters;
           auto wallfacer_create_op = operation(op);
           current_params.current_fees->set_fee(wallfacer_create_op);
           ...
           prop_op.proposed_ops.emplace_back(wallfacer_create_op);
   ...
   full_transaction update_wallfacer_formal(string proposing_account, map<account_id_type, account_id_type> replace_queue, int64_t expiration_time,
	   bool broadcast /* = false */)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   wallfacer_member_update_operation op;
		   auto wallfacer_member_account = get_wallfacer_member(proposing_account);
		   const chain_parameters& current_params = get_global_properties().parameters;
		   op.replace_queue = replace_queue;
		   auto wallfacer_update_op = operation(op);
		   current_params.current_fees->set_fee(wallfacer_update_op);
		   ...
		   prop_op.proposer = get_account(proposing_account).get_id();
		   prop_op.fee_paying_account = get_account(proposing_account).addr;
		   prop_op.proposed_ops.emplace_back(wallfacer_update_op);
   ...
   full_transaction wallfacer_cancel_publisher(const string& account, const account_id_type publisher, const string& symbol, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
	...
	full_transaction wallfacer_change_eth_gas_price(const string& account, const string& gas_price, const string& symbol, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   wallfacer_change_eth_gas_price_operation op;
		   auto wallfacer_member_account = get_wallfacer_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction wallfacer_appointed_lockbalance_wallfacer(const string& account, const std::map<string, asset>& lockbalance, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   set_wallfacer_lockbalance_operation op;
		   auto wallfacer_member_account = get_wallfacer_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction wallfacer_determine_block_payment(const string& account, const std::map<uint32_t, uint32_t>& blocks_pays, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   wallfacer_determine_block_payment_operation op;
		   auto wallfacer_member_account = get_wallfacer_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction proposal_block_address(const string& account, const fc::flat_set<address>& block_addr, int64_t expiration_time, bool broadcast)
   {
	   try {
		   ...
		   prop_op.fee_paying_account = get_account(account).addr;
		   prop_op.proposed_ops.emplace_back(op);
		   prop_op.type = vote_id_type::wallfacer;
		   current_params.current_fees->set_fee(prop_op.proposed_ops.back().op);
		   tx.operations.push_back(prop_op);
		   set_operation_fees(tx, current_params.current_fees);
		   tx.validate();
   ...
   full_transaction add_whiteOperation(const string& proposer, const address& addr, const fc::flat_set<int>& ops, int64_t expiration_time, bool broadcast)
   {
	   try {
		   ...
		   auto wallfacer_member_account = get_wallfacer_member(proposer);
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction wallfacer_determine_withdraw_deposit(const string& account, bool can,const string& symbol, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   wallfacer_determine_withdraw_deposit_operation op;
		   auto wallfacer_member_account = get_wallfacer_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
		   op.can = can;
		   op.symbol = symbol;
   ...
   wallfacer_member_object get_wallfacer_member(string owner_account)
   {
      try
      {
         fc::optional<wallfacer_member_id_type> committee_member_id = maybe_id<wallfacer_member_id_type>(owner_account);
         if (committee_member_id)
         {
            std::vector<wallfacer_member_id_type> ids_to_get;
            ids_to_get.push_back(*committee_member_id);
            std::vector<fc::optional<wallfacer_member_object>> wallfacer_member_objects = _remote_db->get_wallfacer_members(ids_to_get);
            if (wallfacer_member_objects.front())
               return *wallfacer_member_objects.front();
         ...
         else
         {
            ...
            {
               account_id_type owner_account_id = get_account_id(owner_account);
               fc::optional<wallfacer_member_object> committee_member = _remote_db->get_wallfacer_member_by_account(owner_account_id);
               if (committee_member)
   ...
   full_transaction cancel_cold_hot_uncreate_transaction(const string& proposer, const string& trxid, const int64_t& exception_time, bool broadcast) {
	   try {
		   ...
		   auto wallfacer_id = wallfacer_obj.wallfacer_member_account;
		   auto wallfacer_account_obj = get_account(wallfacer_id);
		   coldhot_canecl_transfer_op.wallfacer = wallfacer_account_obj.addr;
		   coldhot_canecl_transfer_op.wallfacer_id = wallfacer_obj.id;
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction refund_request(const string& refund_account, const string& txid, bool broadcast)
   {
	   try {
		   ...
		   wallfacer_refund_balance_operation op;
		   op.refund_addr = addr;
		   op.txid = txid;
   ...
   full_transaction refund_uncombined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	   FC_ASSERT(!is_locked());
	   ...
	   auto account = get_account(wallfacer);
	   auto wallfacer_obj = get_wallfacer_member(wallfacer);
	   auto addr = account.addr;
	   FC_ASSERT(addr != address(), "wallet doesnt has this account.");
	   FC_ASSERT(wallfacer_obj.wallfacer_member_account == account.id, "wallfacer account error");
	   proposal_create_operation prop_op;
	   prop_op.expiration_time = fc::time_point_sec(time_point::now()) + fc::seconds(expiration_time);
	   prop_op.proposer = get_account(wallfacer).get_id();
	   prop_op.fee_paying_account = addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   wallfacer_refund_crosschain_trx_operation wallfacer_refund_op;
	   wallfacer_refund_op.not_enough_sign_trx_id = transaction_id_type(txid);
	   wallfacer_refund_op.wallfacer_address = addr;
	   wallfacer_refund_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
	   prop_op.proposed_ops.emplace_back(wallfacer_refund_op);
	   current_params.current_fees->set_fee(prop_op.proposed_ops.back().op);
	   signed_transaction trx;
	   trx.operations.emplace_back(prop_op);
	   set_operation_fees(trx, current_params.current_fees);
	...
	full_transaction wallfacer_pass_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	   FC_ASSERT(!is_locked());
	   FC_ASSERT(transaction_id_type(txid) != transaction_id_type(), "txid is not legal");
	   ...
	   auto account = get_account(wallfacer);
	   auto wallfacer_obj = get_wallfacer_member(wallfacer);
	   FC_ASSERT(wallfacer_obj.wallfacer_type == PERMANENT,"Only Permanent wallfacer can use this proposal");
	   auto addr = account.addr;
	   FC_ASSERT(addr != address(), "wallet doesnt has this account.");
	   FC_ASSERT(wallfacer_obj.wallfacer_member_account == account.id, "wallfacer account error");
	   proposal_create_operation prop_op;
	   ...
	   prop_op.fee_paying_account = addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   wallfacer_pass_success_trx_operation wallfacer_refund_op;
	   wallfacer_refund_op.pass_transaction_id = transaction_id_type(txid);
	   wallfacer_refund_op.wallfacer_address = addr;
	   wallfacer_refund_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
	   prop_op.proposed_ops.emplace_back(wallfacer_refund_op);
	...
	full_transaction cancel_eth_sign_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
		FC_ASSERT(!is_locked());
		FC_ASSERT(transaction_id_type(txid) != transaction_id_type(), "txid is not legal");
		auto without_sign_trx = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_eth_wallfacer_need_sign, transaction_id_type(txid));
		FC_ASSERT(without_sign_trx.size() == 1, "Transaction find error");
		auto account = get_account(wallfacer);
		auto wallfacer_obj = get_wallfacer_member(wallfacer);
		auto addr = account.addr;
		...
		prop_op.fee_paying_account = addr;
		prop_op.type = vote_id_type::vote_type::cancel_commit;
		eths_cancel_unsigned_transaction_operation wallfacer_refund_op;
		wallfacer_refund_op.cancel_trx_id = transaction_id_type(txid);
		wallfacer_refund_op.wallfacer_address = addr;
		wallfacer_refund_op.wallfacer_id = wallfacer_obj.id;
		const chain_parameters& current_params = get_global_properties().parameters;
		prop_op.proposed_ops.emplace_back(wallfacer_refund_op);
   ...
   full_transaction refund_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	   FC_ASSERT(!is_locked());
	   FC_ASSERT(transaction_id_type(txid) != transaction_id_type(), "txid is not legal");
	   auto without_sign_trx = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_combine_trx_create, transaction_id_type(txid));
	   FC_ASSERT(without_sign_trx.size() == 1, "Transaction find error");
	   ...
	   prop_op.fee_paying_account = addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   wallfacer_cancel_combine_trx_operation wallfacer_refund_op;
	   wallfacer_refund_op.fail_trx_id = transaction_id_type(txid);
	   wallfacer_refund_op.wallfacer_address = addr;
	   wallfacer_refund_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
	   prop_op.proposed_ops.emplace_back(wallfacer_refund_op);
   ...
   full_transaction eth_cancel_fail_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	   FC_ASSERT(!is_locked());
	   FC_ASSERT(transaction_id_type(txid) != transaction_id_type(), "txid is not legal");
	   auto without_sign_trx = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_eth_wallfacer_sign, transaction_id_type(txid));
	   FC_ASSERT(without_sign_trx.size() == 1, "Transaction find error");
	   auto account = get_account(wallfacer);
	   auto wallfacer_obj = get_wallfacer_member(wallfacer);
	   auto addr = account.addr;
	   ...
	   eth_cancel_op.wallfacer_address = addr;
	   eth_cancel_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction cancel_coldhot_eth_fail_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast = false) {
	   FC_ASSERT(!is_locked());
	   FC_ASSERT(transaction_id_type(txid) != transaction_id_type(), "txid is not legal");
	   auto without_sign_trx = _remote_db->get_coldhot_transaction(coldhot_trx_state::coldhot_eth_wallfacer_sign, transaction_id_type(txid));
	   FC_ASSERT(without_sign_trx.size() == 1, "Transaction find error");
	   auto account = get_account(wallfacer);
	   auto wallfacer_obj = get_wallfacer_member(wallfacer);
	   auto addr = account.addr;
	   ...
	   cancel_op.wallfacer = addr;
	   cancel_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
	   prop_op.proposed_ops.emplace_back(cancel_op);
	   current_params.current_fees->set_fee(prop_op.proposed_ops.back().op);
	   signed_transaction trx;
   ...
   full_transaction wallfacer_change_acquire_trx(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast = false) {
	   ...
	   prop_op.fee_paying_account = addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   wallfacer_change_acquire_trx_operation change_op;
	   change_op.change_transaction_id = transaction_id_type(txid);
	   change_op.wallfacer_address = addr;
	   change_op.wallfacer_id = wallfacer_obj.id;
	   const chain_parameters& current_params = get_global_properties().parameters;
	   prop_op.proposed_ops.emplace_back(change_op);
	   current_params.current_fees->set_fee(prop_op.proposed_ops.back().op);
	   signed_transaction trx;
	   trx.operations.emplace_back(prop_op);
	   set_operation_fees(trx, current_params.current_fees);
   ...
   crosschain_prkeys create_crosschain_symbol_cold(const string &real_symbol,const string& out_key_file, const string& encrypt_key) {
	   try {
		   ...
		   std::string wif_key;
		   if (bGuard) {
			   wif_key = fd->create_normal_account("wallfacer");
		   }
		   else {
   ...
   full_transaction update_asset_private_with_coldkeys(const string& from_account, const string& symbol, const string& cold_address, const string& cold_pubkey, bool broadcast) {
	   try {
		   FC_ASSERT(!is_locked());
		   auto wallfacer_account = get_wallfacer_member(from_account);
		   FC_ASSERT(wallfacer_account.wallfacer_member_account != account_id_type(), "only wallfacer member can do this operation.");
		   auto asset_id = get_asset_id(symbol);
		   crosschain_prkeys hot_keys;
		   if (symbol == "ETH" || symbol.find("ERC") != symbol.npos)
		   {
			   hot_keys = create_crosschain_symbol(symbol + "|etwallfacer");
		   }
		   else
		   {
			   auto obj = get_current_multi_address(symbol);
			   FC_ASSERT(obj.valid());
			   auto mutlisig_obj = get_current_multi_address_obj(symbol, wallfacer_account.wallfacer_member_account);
		   ...
		   op.addr = get_account(wallfacer_account.wallfacer_member_account).addr;
		   op.account_id = get_account(wallfacer_account.wallfacer_member_account).get_id();
   ...
   full_transaction update_asset_private_keys(const string& from_account, const string& symbol,const string& out_key_file,const string& encrypt_key, bool broadcast,bool use_brain_key=false)
   {
	   try {
		   ...
		   FC_ASSERT(wallfacer_account.wallfacer_member_account != account_id_type(),"only wallfacer member can do this operation.");
		   auto asset_id = get_asset_id(symbol);
		   auto  hot_keys =create_crosschain_symbol(symbol+"|etwallfacer",false,use_brain_key);
		   //string hot_pri = cross_interface->export_private_key(symbol, "");
		   auto cold_keys = create_crosschain_symbol(symbol + "|etwallfacer",true, use_brain_key);
   ...
   vector<optional<multisig_address_object>> get_multi_account_wallfacer(const string & multi_address, const string& symbol) {
	   get_asset_id(symbol);
	   return _remote_db->get_multi_account_wallfacer(multi_address,symbol);
   }
   full_transaction transfer_from_cold_to_hot(const string& proposer, const string& from_account, const string& to_account, const string& amount, const string& asset_symbol,const string& memo,const int64_t& expiration_time, bool broadcast)
   {
	   try
	   {
		   ...
		   auto wallfacer_id = wallfacer_obj.wallfacer_member_account;
		   auto wallfacer_account_obj = get_account(wallfacer_id);
		   coldhot_transfer_op.wallfacer = wallfacer_account_obj.addr;
		   coldhot_transfer_op.wallfacer_id = wallfacer_obj.id;
   ...
   {
	   try {
		   return _remote_db->get_multisig_address_obj(symbol,wallfacer);
	   }FC_CAPTURE_AND_RETHROW()
   }
   ...
   full_transaction sign_multi_asset_trx(const string& account, multisig_asset_transfer_id_type id,const string& wallfacer, bool broadcast)
   {
	   try {
		   ...
		   wallfacer_member_object wallfacer_obj = get_wallfacer_member(wallfacer);
		   vector<optional<multisig_address_object>> multisig_obj = _remote_db->get_multisig_address_obj(multisig_trx_obj.chain_type,wallfacer_obj.wallfacer_member_account);
   ...
   full_transaction account_change_for_crosschain(const string& proposer,const string& symbol,const string& hot,const string& cold, int64_t expiration_time,bool broadcast)
   {
	   try 
	   {
		   ...
		   wallfacer_update_multi_account_operation update_op;
   ...
   { try {
      ...
      fc::optional<wallfacer_member_object> committee_member_obj = _remote_db->get_wallfacer_member_by_account(committee_member_owner_account_id);
   ...
   {
	   auto vec_objs = get_multi_address_obj(symbol,wallfacer);
   ...
   {
	   auto wallfacer_ids = get_global_properties().active_committee_members;
	   auto wallfacers = _remote_db->get_wallfacer_members(wallfacer_ids);
	   account_id_type wallfacer_acc;
	   for (auto wallfacer : wallfacers)
	   {
		   if (wallfacer.valid())
		   {
			   if (wallfacer->formal == true)
			   {
				   wallfacer_acc = wallfacer->wallfacer_member_account;
				   break;
			   }
		   }
	   }
	   auto ret = get_current_multi_address_obj(symbol, wallfacer_acc);
	   if (!ret.valid())
   ...
   void wallfacer_sign_eths_multi_account_create_trx(const string& tx_id, const string& wallfacer,const string& keyfile,const string&decryptkey) {
		FC_ASSERT(!is_locked());
		auto wallfacer_obj = get_wallfacer_member(wallfacer);
		const account_object & account_obj = get_account(wallfacer);
		//get transaction and eth crosschain plugin
		FC_ASSERT(transaction_id_type(tx_id) != transaction_id_type(), "not correct transction.");
		auto trx_range = _remote_db->get_eths_multi_create_account_trx(eth_multi_account_trx_state::sol_create_need_wallfacer_sign, transaction_id_type(tx_id));
		...
		FC_ASSERT(wallfacer_obj.id == multi_account_op.wallfacer_to_sign);
		auto & manager = graphene::crosschain::crosschain_manager::get_instance();
		...
		FC_ASSERT(_crosschain_keys.count(multi_account_op.wallfacer_sign_hot_address) > 0, "private key doesnt belong to this wallet.");
		...
		FC_ASSERT(_crosschain_keys.count(multi_account_op.wallfacer_sign_cold_address) > 0|| decrypted_keys.find(multi_account_op.chain_type+multi_account_op.wallfacer_sign_cold_address)!= decrypted_keys.end(), "private key doesnt belong to this wallet or keyfile.");
		string wif_key_cold;
		if(_crosschain_keys.count(multi_account_op.wallfacer_sign_cold_address) > 0)
			wif_key_cold=_crosschain_keys[multi_account_op.wallfacer_sign_cold_address].wif_key;
		else
		{
			//std::cout << fc::json::to_pretty_string(decrypted_keys) << std::endl << multi_account_op.chain_type + multi_account_op.wallfacer_sign_cold_address << std::endl;
			wif_key_cold = decrypted_keys[multi_account_op.chain_type+multi_account_op.wallfacer_sign_cold_address].wif_key;
		}
		auto wif_key_hot = _crosschain_keys[multi_account_op.wallfacer_sign_hot_address].wif_key;
		...
		eths_multi_sol_wallfacer_sign_operation tx_op;
		tx_op.wallfacer_to_sign = multi_account_op.wallfacer_to_sign;
		tx_op.wallfacer_sign_address = account_obj.addr;
		tx_op.multi_hot_sol_wallfacer_sign = siging_hot;
		tx_op.multi_cold_sol_wallfacer_sign = siging_cold;
   ...
   string get_coldhot_trx_sig(const string& tx_id, const string& wallfacer, const string& keyfile, const string& decryptkey)
   {
	   ...
	   string config = (*_crosschain_manager)->get_config();
	   crosschain_plugin->initialize_config(fc::json::from_string(config).get_object());
	   string temp_wallfacer(wallfacer);
	   ...
	   FC_ASSERT(handled_trx.trxs.size() == 1, "Transcation turn error in wallfacer sign cold hot transaction");
	   auto multi_objs = _remote_db->get_multisig_account_pair(coldhot_op.asset_symbol);
	   string redeemScript = "";
	   string wallfacer_address = "";
	   bool cold = false;
	   const account_object & account_obj = get_account(wallfacer);
	   const auto& wallfacer_obj = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
	   auto wallfacer_multi_address_objs = get_multi_address_obj(coldhot_op.asset_symbol, account_obj.id);
	   ...
	   for (auto multi_obj : multi_objs) {
		   if (multi_obj->bind_account_hot == from_account) {
			   ...
			   for (auto wallfacer_multi_address_obj : wallfacer_multi_address_objs) {
				   if (wallfacer_multi_address_obj->multisig_account_pair_object_id == multi_obj->id) {
					   wallfacer_address = wallfacer_multi_address_obj->new_address_hot;
					   break;
				   }
			   }
			   break;
		   ...
		   if (multi_obj->bind_account_cold == from_account) {
			   ...
			   for (auto wallfacer_multi_address_obj : wallfacer_multi_address_objs) {
				   if (wallfacer_multi_address_obj->multisig_account_pair_object_id == multi_obj->id) {
					   wallfacer_address = wallfacer_multi_address_obj->new_address_cold;
					   cold = true;
					   break;
				   }
			   }
	   ...
	   FC_ASSERT((redeemScript != "") && (wallfacer_address != ""), "redeemScript exist error");
	   ...
	   if (!cold)
	   {
		   FC_ASSERT(_crosschain_keys.count(wallfacer_address) > 0, "private key doesnt belong to this wallet.");
		   auto wif_key = _crosschain_keys[wallfacer_address].wif_key;
		   auto key_ptr = prk_ptr->import_private_key(wif_key);
		   prk_ptr->set_key(*key_ptr);
	   }
	   else
	   {
		   ...
		   FC_ASSERT(keys.find(coldhot_op.asset_symbol + wallfacer_address) != keys.end(), "cold key can't be found in keyfile");
		   auto key_ptr = prk_ptr->import_private_key(keys[coldhot_op.asset_symbol + wallfacer_address].wif_key);
		   prk_ptr->set_key(*key_ptr);
	   }
	   string siging;
	   if (coldhot_op.asset_symbol == "ETH" || coldhot_op.asset_symbol.find("ERC") != coldhot_op.asset_symbol.npos) {
   ...
   void send_coldhot_transfer_with_sign(const string& tx_id, const string& wallfacer, const string& siging) {
	   ...
	   tx_op.sign_wallfacer = wallfacer_obj->id;
	   tx_op.asset_symbol = coldhot_op.asset_symbol;
	   tx_op.wallfacer_address = account_obj.addr;
   ...
   void wallfacer_sign_coldhot_transaction(const string& tx_id, const string& wallfacer,const string& keyfile,const string& decryptkey) {
	   ...
	   else
	   {
		   ...
		   FC_ASSERT(keys.find(coldhot_op.asset_symbol + wallfacer_address) != keys.end(),"cold key can't be found in keyfile");
		   //std::cout << fc::json::to_pretty_string(keys) << std::endl << coldhot_op.asset_symbol + wallfacer_address << std::endl;
		   auto key_ptr = prk_ptr->import_private_key(keys[coldhot_op.asset_symbol+wallfacer_address].wif_key);
		   prk_ptr->set_key(*key_ptr);
	   }
	   string siging;
   ...
   void wallfacer_sign_eths_coldhot_final_trx(const string& trx_id, const string & wallfacer, const string& keyfile, const string&decryptkey) {
	   FC_ASSERT(!is_locked());
	   FC_ASSERT(transaction_id_type(trx_id) != transaction_id_type(), "not correct transction.");
	   auto trx = _remote_db->get_coldhot_transaction(coldhot_trx_state::coldhot_eth_wallfacer_need_sign, transaction_id_type(trx_id));
	   FC_ASSERT(trx.size() == 1, "Transaction find error");
	   ...
	   auto sign_wallfacer = eth_without_sign_trx_obj["signer"].as_string();
	   ...
	   FC_ASSERT(_crosschain_keys.count(sign_wallfacer) > 0|| decrypted_keys.find(coldhot_op.asset_symbol + sign_wallfacer) != decrypted_keys.end(), "private key doesnt belong to this wallet or keyfile.");
	   if (_crosschain_keys.count(sign_wallfacer) > 0)
		   wif_key = _crosschain_keys[sign_wallfacer].wif_key;
	   else
	   {
		   //std::cout << fc::json::to_pretty_string(decrypted_keys) << std::endl << coldhot_op.asset_symbol + sign_wallfacer << std::endl;
		   wif_key = decrypted_keys[coldhot_op.asset_symbol + sign_wallfacer].wif_key;
	   }
	   ...
	   eths_coldhot_wallfacer_sign_final_operation trx_op;
	   trx_op.signed_crosschain_trx_id = hdtx.trx_id;
	   account_object account_obj = get_account(wallfacer);
	   const auto& wallfacer_obj = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
	   trx_op.wallfacer_to_sign = wallfacer_obj->id;
	   ...
	   trx_op.wallfacer_address = account_obj.addr;
	   signed_transaction transaction;
   ...
   void wallfacer_changer_eth_coldhot_singer_trx(const string wallfacer, const string txid, const string& newaddress, const int64_t& expiration_time, const string& keyfile, const string& decryptkey, bool broadcast) {
	   ...
	   auto trx = _remote_db->get_coldhot_transaction(coldhot_trx_state::coldhot_eth_wallfacer_need_sign, transaction_id_type(txid));
	   ...
	   auto sign_wallfacer = newaddress;
	   auto without_sign = eth_without_sign_trx_obj["without_sign"].as_string();
	   ...
	   FC_ASSERT(_crosschain_keys.count(sign_wallfacer) > 0|| decrypted_keys.find(withop_without_sign.asset_symbol+ sign_wallfacer)!= decrypted_keys.end(), "private key doesnt belong to this wallet.");
	   string wif_key;
	   if(_crosschain_keys.count(sign_wallfacer) > 0)
		   wif_key = _crosschain_keys[sign_wallfacer].wif_key;
	   else
	   {
		   wif_key = decrypted_keys[withop_without_sign.asset_symbol + sign_wallfacer].wif_key;
	   }
	   auto key_ptr = prk_ptr->import_private_key(wif_key);
	   FC_ASSERT(key_ptr.valid());
	   ...
	   prop_op.expiration_time = fc::time_point_sec(time_point::now()) + fc::seconds(expiration_time);
	   prop_op.proposer = get_account(wallfacer).get_id();
	   prop_op.fee_paying_account = get_account(wallfacer).addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   eths_wallfacer_coldhot_change_signer_operation trx_op;
	   trx_op.signed_crosschain_trx_id = hdtx.trx_id;
	   account_object account_obj = get_account(wallfacer);
	   const auto& wallfacer_obj = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
	   trx_op.wallfacer_to_sign = wallfacer_obj->id;
   ...
   void wallfacer_changer_eth_singer_trx(const string wallfacer, const string txid, const string& newaddress, const int64_t& expiration_time, bool broadcast) {
	   ...
	   auto trx = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_eth_wallfacer_need_sign, transaction_id_type(txid));
	   FC_ASSERT(trx.size() == 1, "Transaction error");
	   ...
	   auto without_sign = eth_without_sign_trx_obj["without_sign"].as_string();
	   auto prk_ptr = graphene::privatekey_management::crosschain_management::get_instance().get_crosschain_prk(withop_without_sign.asset_symbol);
	   FC_ASSERT(_crosschain_keys.count(sign_wallfacer) > 0, "private key doesnt belong to this wallet.");
	   auto wif_key = _crosschain_keys[sign_wallfacer].wif_key;
	   auto key_ptr = prk_ptr->import_private_key(wif_key);
	   FC_ASSERT(key_ptr.valid());
	   string siging = hdl->sign_multisig_transaction(fc::variant_object("change_signer_sign", without_sign), prk_ptr, "", false);
	   std::cout << siging << std::endl;
	   ...
	   prop_op.fee_paying_account = get_account(wallfacer).addr;
	   prop_op.type = vote_id_type::vote_type::cancel_commit;
	   eths_wallfacer_change_signer_operation trx_op;
	   trx_op.signed_crosschain_trx_id = hdtx.trx_id;
	   account_object account_obj = get_account(wallfacer);
	   const auto& wallfacer_obj = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
	   trx_op.wallfacer_to_sign = wallfacer_obj->id;
   ...
   void wallfacer_sign_eths_final_trx(const string& trx_id, const string & wallfacer) {
	   FC_ASSERT(!is_locked());
	   auto wallfacer_obj = get_wallfacer_member(wallfacer);
	   auto wallfacer_id = wallfacer_obj.wallfacer_member_account;
	   /*
	   if (trx_id == "ALL") {
		   auto trxs = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_eth_wallfacer_need_sign, transaction_id_type());
			   ...
			   hdl->initialize_config(fc::json::from_string(config).get_object());
			   string temp_wallfacer(wallfacer);
			   auto current_multi_obj = get_current_multi_address_obj(withop_without_sign.asset_symbol, wallfacer_id);
			   ...
			   eths_wallfacer_sign_final_operation trx_op;
			   account_object account_obj = get_account(wallfacer);
			   const auto& wallfacer_obj = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
			   trx_op.wallfacer_to_sign = wallfacer_obj->id;
	   ...
	   */
		   auto trx = _remote_db->get_crosschain_transaction(transaction_stata::withdraw_eth_wallfacer_need_sign, transaction_id_type(trx_id));
		   FC_ASSERT(trx.size() == 1, "Transaction error");
		   auto& op = trx[0].real_transaction.operations[0];
		   auto withop_without_sign = op.get<crosschain_withdraw_combine_sign_operation>();
		   auto& manager = graphene::crosschain::crosschain_manager::get_instance();
		   ...
		   trx_op.signed_crosschain_trx_id = hdtx.trx_id;
		   account_object account_obj = get_account(wallfacer);
		   const auto& wallfacer_obj_x = _remote_db->get_wallfacer_member_by_account(account_obj.get_id());
		   trx_op.wallfacer_to_sign = wallfacer_obj_x->id;
		   trx_op.chain_type = withop_without_sign.asset_symbol;
		   trx_op.combine_trx_id = transaction_id_type(trx_id);
		   trx_op.signed_crosschain_trx = siging;
		   trx_op.wallfacer_address = account_obj.addr;
	   ...
	   if (trx_id == "ALL"){
		   ...
		   for (const auto& trx : trxs) {
			  ...
			  // string siging = hdl->sign_multisig_transaction(withop_without_sign.withdraw_source_trx, prk_ptr, account_pair_obj->redeemScript_hot, false);
			   ...
			   trx_op.sign_wallfacer = wallfacer_obj->id;
			   trx_op.wallfacer_address = account_obj.addr;
			   signed_transaction transaction;
			   transaction.operations.push_back(trx_op);
			   set_operation_fees(transaction, _remote_db->get_global_properties().parameters.current_fees);
   ...
   full_transaction wallfacer_lock_balance(string wallfacer_account,
	   ...
	   try {
		   ...
		   wallfacer_lb_op.lock_asset_amount = asset_obj->amount_from_string(amount).amount;
		   wallfacer_lb_op.lock_balance_account = wallfacer_obj->id;
		   wallfacer_lb_op.lock_balance_account_id = wallfacer_obj->wallfacer_member_account;
		   wallfacer_lb_op.lock_address = iter.find(wallfacer_account)->addr;
		   signed_transaction tx;

		   tx.operations.push_back(wallfacer_lb_op);
		   set_operation_fees(tx, _remote_db->get_global_properties().parameters.current_fees);
		   tx.validate();
   ...
   full_transaction wallfacer_foreclose_balance(string wallfacer_account,
	   ...
	   try {
		   ...
		   wallfacer_fcb_op.foreclose_asset_amount = asset_obj->amount_from_string(amount).amount;
		   wallfacer_fcb_op.foreclose_balance_account = wallfacer_obj->id;
		   wallfacer_fcb_op.foreclose_balance_account_id = iter.find(wallfacer_account)->get_id();
		   wallfacer_fcb_op.foreclose_address = iter.find(wallfacer_account)->addr;
		   signed_transaction tx;

		   tx.operations.push_back(wallfacer_fcb_op);
		   set_operation_fees(tx, _remote_db->get_global_properties().parameters.current_fees);
		   tx.validate();
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
	...
	full_transaction propose_wallfacer_pledge_change(
	  ...
	  bool broadcast = false)
	{
	  FC_ASSERT(changed_values.contains("minimum_wallfacer_pledge_line"));
	  variant_object temp_asset_set1 = changed_values.find("minimum_wallfacer_pledge_line")->value().get_object();
	  const chain_parameters& current_params = get_global_properties().parameters;
	  chain_parameters new_params = current_params;
	  for (const auto& item : temp_asset_set1) {
		  new_params.minimum_wallfacer_pledge_line[item.key()] = asset(item.value().as_uint64(), get_asset_id(item.key()));
	  }
	...
	full_transaction propose_pay_back_asset_rate_change(
		...
		proposal_create_operation prop_op;
		auto wallfacer_obj = get_wallfacer_member(proposing_account);
		prop_op.proposer = get_account(proposing_account).get_id();
		prop_op.fee_paying_account = get_account(proposing_account).addr;
		prop_op.expiration_time = fc::time_point_sec(time_point::now()) + fc::seconds(expiration_time);
...
wallfacer_member_object wallet_api::get_eth_signer(const string& symbol, const string& address) {
	auto wallfacerId = my->get_eth_signer(symbol, address);
	auto wallfacer_member_objects = my->_remote_db->get_wallfacer_member_by_account(wallfacerId);
	if (wallfacer_member_objects.valid())
		return *wallfacer_member_objects;
	return wallfacer_member_object();
	/*
	
	FC_ASSERT(wallfacer_member_objects.size() != 0, "don`t find wallfacer");
	
	auto wallfacer_member = *(wallfacer_member_objects.begin());
	FC_ASSERT(wallfacer_member.valid(), "don`t find wallfacer");
	return *wallfacer_member;
	*/
}
...
void wallet_api::send_coldhot_transfer_with_sign(const string& tx_id, const string& wallfacer, const string& siging)
{
	return my->send_coldhot_transfer_with_sign(tx_id, wallfacer, siging);
}
std::string wallet_api::get_coldhot_trx_sig(const string& tx_id, const string& wallfacer, const string& keyfile, const string& decryptkey)
{
	return my->get_coldhot_trx_sig(tx_id, wallfacer, keyfile, decryptkey);
}
...
void wallet_api::wallfacer_sign_coldhot_transaction(const string& tx_id, const string& wallfacer, const string& keyfile, const string& decryptkey) {
	return my->wallfacer_sign_coldhot_transaction(tx_id, wallfacer, keyfile,decryptkey);
}

void wallet_api::wallfacer_sign_eths_multi_account_create_trx(const string& tx_id, const string& wallfacer, const string& keyfile, const string& decryptkey) {
	my->wallfacer_sign_eths_multi_account_create_trx(tx_id, wallfacer,keyfile,decryptkey);
}
void wallet_api::wallfacer_sign_eths_final_trx(const string& trx_id, const string & wallfacer) {
	my->wallfacer_sign_eths_final_trx(trx_id, wallfacer);
}
void wallet_api::wallfacer_changer_eth_singer_trx(const string wallfacer, const string txid, const string& newaddress, const int64_t& expiration_time, bool broadcast) {
	my->wallfacer_changer_eth_singer_trx(wallfacer, txid, newaddress, expiration_time,broadcast);
}
void wallet_api::wallfacer_changer_eth_coldhot_singer_trx(const string wallfacer, const string txid, const string& newaddress, const int64_t& expiration_time, const string& keyfile, const string& decryptkey, bool broadcast) {
	my->wallfacer_changer_eth_coldhot_singer_trx(wallfacer, txid, newaddress, expiration_time, keyfile, decryptkey, broadcast);
}

void wallet_api::wallfacer_sign_eths_coldhot_final_trx(const string& trx_id, const string & wallfacer, const string& keyfile, const string& decryptkey) {
	my->wallfacer_sign_eths_coldhot_final_trx(trx_id, wallfacer,keyfile,decryptkey);
}
void wallet_api::wallfacer_sign_crosschain_transaction(const string& trx_id,const string& wallfacer){
	return my->wallfacer_sign_crosschain_transaction(trx_id,wallfacer);
}
optional<multisig_address_object> wallet_api::get_current_multi_address_obj(const string& symbol, const account_id_type& wallfacer) const
{
	return my->get_current_multi_address_obj(symbol,wallfacer);
}
...
full_transaction wallet_api::transfer_wallfacer_multi_account(string multi_account,
	string amount,
	string asset_symbol,
	string multi_to_account,
	string memo,
	bool broadcast/* = false*/){
	return my->transfer_wallfacer_multi_account(multi_account, amount, asset_symbol, multi_to_account,memo, broadcast);
}
full_transaction wallet_api::wallfacer_lock_balance(string wallfacer_account,
	string amount,
	string asset_symbol,
	bool broadcast/* = false*/) {
...
full_transaction wallet_api::wallfacer_foreclose_balance(string wallfacer_account,
	...
	string asset_symbol,
	bool broadcast/* = false*/) {
	return my->wallfacer_foreclose_balance(wallfacer_account, amount, asset_symbol, broadcast);
}
...
full_transaction wallet_api::create_wallfacer_member(string account, bool broadcast /* = false */)
{
   return my->create_wallfacer_member( account, broadcast);
}


full_transaction wallet_api::update_wallfacer_formal(string proposing_account, map<account_id_type, account_id_type> replace_queue,int64_t expiration_time,
	bool broadcast /* = false */)
{
...
full_transaction wallet_api::resign_wallfacer_member(string proposing_account, string account,
...
{
   return my->_remote_db->lookup_wallfacer_member_accounts(lowerbound, limit,false);
}
...
map<string, wallfacer_member_id_type> wallet_api::list_all_wallfacers(const string& lowerbound, uint32_t limit)
{
	return my->_remote_db->lookup_wallfacer_member_accounts(lowerbound, limit,true);
}
...
full_transaction wallet_api::propose_wallfacer_pledge_change(
	const string& proposing_account,
	fc::time_point_sec expiration_time,
	const variant_object& changed_values,
	bool broadcast )
{
	return my->propose_wallfacer_pledge_change(proposing_account, expiration_time, changed_values, broadcast);
...
full_transaction wallet_api::refund_uncombined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->refund_uncombined_transaction(wallfacer,txid,expiration_time, broadcast);
}
full_transaction wallet_api::refund_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->refund_combined_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::cancel_eth_sign_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->cancel_eth_sign_transaction(wallfacer, txid, expiration_time, broadcast);
}

full_transaction wallet_api::wallfacer_pass_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->wallfacer_pass_combined_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::eth_cancel_fail_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->eth_cancel_fail_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::cancel_coldhot_eth_fail_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->cancel_coldhot_eth_fail_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::cancel_coldhot_uncombined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->cancel_coldhot_uncombined_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::cancel_coldhot_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->cancel_coldhot_combined_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::wallfacer_pass_coldhot_combined_transaction(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->wallfacer_pass_coldhot_combined_transaction(wallfacer, txid, expiration_time, broadcast);
}
full_transaction wallet_api::wallfacer_change_acquire_trx(const string wallfacer, const string txid, const int64_t& expiration_time, bool broadcast) {
	return my->wallfacer_change_acquire_trx(wallfacer, txid, expiration_time, broadcast);
}
...
vector<optional<multisig_address_object>> wallet_api::get_multi_account_wallfacer(const string & multi_address,const string& symbol)const {
	return my->get_multi_account_wallfacer(multi_address, symbol);
}
...
vector<optional<multisig_address_object>> wallet_api::get_multi_address_obj(const string& symbol,const account_id_type& wallfacer) const
{
	return my->get_multi_address_obj(symbol,wallfacer);
}
...
full_transaction wallet_api::sign_multi_asset_trx(const string& account, multisig_asset_transfer_id_type id,const string& wallfacer, bool broadcast)
{
	return my->sign_multi_asset_trx(account,id, wallfacer,broadcast);
}
...
full_transaction wallet_api::wallfacer_appointed_publisher(const string& account, const account_id_type publisher, const string& symbol, int64_t expiration_time, bool broadcast)
{
	return my->wallfacer_appointed_publisher(account,publisher,symbol, expiration_time,broadcast);
}
full_transaction wallet_api::wallfacer_cancel_publisher(const string& account, const account_id_type publisher, const string& symbol, int64_t expiration_time, bool broadcast)
{
	return my->wallfacer_cancel_publisher(account, publisher, symbol, expiration_time, broadcast);
}
...
full_transaction wallet_api::wallfacer_appointed_lockbalance_wallfacer(const string& account, const std::map<string, asset>& lockbalance, int64_t expiration_time, bool broadcast)
{
	return my->wallfacer_appointed_lockbalance_wallfacer(account, lockbalance, expiration_time, broadcast);
}
full_transaction wallet_api::wallfacer_determine_block_payment(const string& account, const std::map<uint32_t, uint32_t>& blocks_pays, int64_t expiration_time, bool broadcast)
{
	return my->wallfacer_determine_block_payment(account, blocks_pays,expiration_time,broadcast);
}

full_transaction wallet_api::wallfacer_determine_withdraw_deposit(const string& account, bool can,const string& symbol, int64_t expiration_time, bool broadcast)
{
	return my->wallfacer_determine_withdraw_deposit(account, can, symbol,expiration_time, broadcast);
}
...
std::map<std::string, fc::ntp_info> wallet_api::get_ntp_info()
{
	...
	res["XWC_node"]= my->_remote_db->get_ntp_info();
	res["XWC_client"]= fc::time_point::get_ntp_info();;
	return res;
}
